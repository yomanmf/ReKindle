...
test("both dashboards nest Games inside Misc and two-player games inside Games", function () {
    ["index.html", "index_old.html"].forEach(function (file) {
        ...
        assert.match(source, /miscApps\.push\(\{\s*id: 'folder_games'/);
        assert.doesNotMatch(source, /fragment\.appendChild\(createAppElement\(\{\s*id: 'folder_games'/);
        ...
        assert.match(source, /icons-beta\.js\?v=9/);
    });
});
...
test("Misc is localized and the updated catalog bypasses old service-worker caches", function () {
    ...
    assert.match(read("sw.js"), /rekindle-cache-v46/);
